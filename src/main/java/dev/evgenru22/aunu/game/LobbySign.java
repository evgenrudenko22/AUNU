...
public class LobbySign {
	...
	public static void save() {
		...
		for(LobbySign sign: signs) {
			...
			if(!(sign.getSign().getBlock().getType() == Material.OAK_SIGN))
				return;
	...
	public static void load() {
		...
		for(String str: config.getKeys(false)) {
			...
			if(lobby == null || !(loc.getBlock().getType() == Material.OAK_SIGN))
				return;
		...
		Bukkit.getPluginManager().registerEvents(new Listener() {
			...
			public void clickOnSign(PlayerInteractEvent e) {
				...
				if(block == null || !(block.getType() == Material.OAK_SIGN) || e.getHand() != EquipmentSlot.HAND)
					return;
